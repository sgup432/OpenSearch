...
public class IndicesRequestCacheTests extends OpenSearchSingleNodeTestCase {
    ...
    public void testDeleteAndCreateIndexShardOnSameNodeAndVerifyStats() throws Exception {
        ...
        IndicesRequestCache cache = new IndicesRequestCache(
            Settings.builder().put(IndicesService.INDICES_CACHE_CLEAN_INTERVAL_SETTING.getKey(), TimeValue.timeValueMillis(100000)).build(),
            (shardId -> {
                IndexService indexService1 = null;
                try {
                    indexService1 = indicesService.indexServiceSafe(shardId.getIndex());
                } catch (IndexNotFoundException ex) {
                    return Optional.empty();
                }
                return Optional.of(new IndicesService.IndexShardCacheEntity(indexService1.getShard(shardId.id())));
            }),
            new CacheModule(new ArrayList<>(), Settings.EMPTY).getCacheService(),
            threadPool,
            ClusterServiceUtils.createClusterService(threadPool)
        ...
        BytesReference value = cache.getOrCompute(cacheEntity, loader, reader, termBytes);
        ...
        termBytes = XContentHelper.toXContent(termQuery, MediaTypeRegistry.JSON, false);
        value = cache.getOrCompute(cacheEntity, loader, reader, termBytes);
