...
public class EhCacheDiskCachingTierNew<K, V> implements DiskCachingTier<K, V> {
    ...
    private EhCacheDiskCachingTierNew(Builder<K, V> builder) {
        ...
        this.settings = Objects.requireNonNull(builder.settings);
        this.DISK_WRITE_MINIMUM_THREADS = Setting.intSetting(builder.settingPrefix +
            ".tiered.disk.ehcache.min_threads", 2, 1, 5);
        this.DISK_WRITE_MAXIMUM_THREADS = Setting.intSetting(builder.settingPrefix +
            ".tiered.disk.ehcache.max_threads", 2, 1, 20);
        // Default value is 1 within EhCache.
        this.DISK_WRITE_CONCURRENCY = Setting.intSetting(builder.settingPrefix +
            ".tiered.disk.ehcache.concurrency", 2, 1, 3);
        // Default value is 16 within Ehcache.
        this.DISK_SEGMENTS = Setting.intSetting(builder.settingPrefix +
            ".ehcache.disk.segments", 16, 1, 32);
    ...
    private PersistentCacheManager buildCacheManager() {
        ...
        return CacheManagerBuilder.newCacheManagerBuilder()
            ...
            .using(PooledExecutionServiceConfigurationBuilder.newPooledExecutionServiceConfigurationBuilder()
                ...
                // event listeners
                .pool(this.threadPoolAlias, DISK_WRITE_MINIMUM_THREADS.get(settings),
                    DISK_WRITE_MAXIMUM_THREADS.get(settings)).build())
            .build(true);
    }

    private Cache<K, V> buildCache(Duration expireAfterAccess, Builder<K, V> builder) {
        return this.cacheManager.createCache(DISK_CACHE_ALIAS,
            CacheConfigurationBuilder.newCacheConfigurationBuilder(this.keyType,
            this.valueType,
                ResourcePoolsBuilder.newResourcePoolsBuilder().disk(maxWeightInBytes, MemoryUnit.B)).withExpiry(new ExpiryPolicy<K, V>() {
                @Override
                ...
                .withService(new OffHeapDiskStoreConfiguration(this.threadPoolAlias, DISK_WRITE_CONCURRENCY.get(settings),
                    DISK_SEGMENTS.get(settings))));
    }

    private CacheEventListenerConfigurationBuilder getListenerConfiguration(Builder<K, V> builder) {
        CacheEventListenerConfigurationBuilder configurationBuilder = CacheEventListenerConfigurationBuilder
            .newEventListenerConfiguration(this.ehCacheEventListener,
                EventType.EVICTED,
                EventType.EXPIRED,
                EventType.REMOVED,
                EventType.UPDATED,
...
//        try {
//            cacheManager.destroy(); // Cleans up all the data associated with the cache.
//        } catch (CachePersistenceException e) {
//            throw new OpenSearchException("Exception occurred while destroying disk cache", e);
//        }
    }
        ...
        public void onEvent(CacheEvent<? extends K, ? extends V> event) {
            switch (event.getType()) {
                case CREATED:
                    ...
                    break;
                case EVICTED:
                    this.removalListener.ifPresent(listener -> listener.onRemoval(new RemovalNotification<>(event.getKey(),
                        event.getOldValue(),
    ...
    public static class Builder<K, V> {
        ...
        public EhCacheDiskCachingTierNew.Builder<K, V> setMaximumWeightInBytes(long sizeInBytes) {
            //assert sizeInBytes > 1024*100;
            this.maxWeightInBytes = sizeInBytes;
            return this;
        }

        public EhCacheDiskCachingTierNew.Builder<K, V> setExpireAfterAccess(TimeValue expireAfterAcess) {
            this.expireAfterAcess = expireAfterAcess;
            return this;
        }

        public EhCacheDiskCachingTierNew.Builder<K, V> setKeyType(Class<K> keyType) {
            this.keyType = keyType;
            return this;
        }

        public EhCacheDiskCachingTierNew.Builder<K, V> setValueType(Class<V> valueType) {
            this.valueType = valueType;
            return this;
        }

        public EhCacheDiskCachingTierNew.Builder<K, V> setStoragePath(String storagePath) {
            this.storagePath = storagePath;
            return this;
        }

        public EhCacheDiskCachingTierNew.Builder<K, V> setThreadPoolAlias(String threadPoolAlias) {
            this.threadPoolAlias = threadPoolAlias;
            return this;
        }

        public EhCacheDiskCachingTierNew.Builder<K, V> setSettings(Settings settings) {
            this.settings = settings;
            return this;
        }

        public EhCacheDiskCachingTierNew.Builder<K, V> setDiskCacheAlias(String diskCacheAlias) {
            this.diskCacheAlias = diskCacheAlias;
            return this;
        }

        public EhCacheDiskCachingTierNew.Builder<K, V> setSettingPrefix(String settingPrefix) {
            this.settingPrefix = settingPrefix;
            return this;
        }

        public EhCacheDiskCachingTierNew.Builder<K, V> setIsEventListenerModeSync(boolean isEventListenerModeSync) {
            this.isEventListenerModeSync = isEventListenerModeSync;
            return this;
        }

        public EhCacheDiskCachingTierNew<K, V> build() {
            return new EhCacheDiskCachingTierNew<>(this);
        }
    }
}
